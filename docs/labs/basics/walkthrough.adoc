...
[time=3]
[id="section-two"]
== Execute an inference using a base model
...
+
----
deployment.apps/lab-tf-server created
service/lab-tf-server created
route.route.openshift.io/lab-tf-server created
----
...
From the topology view, follow the steps below to visually validate your server started successfully:

. Click on the `lab-tf-server` deployment
...
Let's run the test. +
Switch back to the _JupyterLab_ environment and inspect the following resource:
...
[NOTE]
--
Be patient, the training process should take between 2-4 minutes.
...
[type=verificationSuccess]
Congratulations, you've created a custom AI model !!

[type=verificationFail]
Review the steps in this exercise to try to understand the point cause of failure, then retry.
