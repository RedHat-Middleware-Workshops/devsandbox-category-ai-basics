...
The relevant part of our test is the following:
...
Process above is easily readable. These are the highlights:

. The _HTTP_ listener serves the request
. After cleaning incoming headers, the process invokes the _Model Server_
...
If you feel curious about the source code YAML for the above _Apache Camel_ route, you can also open the file with a single mouse click. The YAML view is easy to read and describes the process in a friendly manner.
